 ...
 */
export const CreateAssessmentQuestionOperationSchema = CreateAssessmentQuestionSchema.extend({
    // Additional validation for create operations
    question_text: z
        .string()
        .min(1)
 ...
 */
export const UpdateAssessmentQuestionOperationSchema = UpdateAssessmentQuestionSchema.extend({
// Ensure at least one field is provided for update
}).refine(data => Object.keys(data).length > 0, {
    message: 'At least one field must be provided for update',
});
 ...
 */
export const ListUserAssessmentsOperationSchema = UserAssessmentQuerySchema.extend({
    // Pagination
    page: z.number().int().min(1).default(1),
    limit: z.number().int().min(1).max(100).default(20),
    offset: z.number().int().min(0).default(0),
 ...
 */
export const CreateAssessmentResponseOperationSchema = CreateAssessmentResponseSchema.extend({
    // Additional validation for create operations
    response_value: z.number().int().optional(),
    response_text: z.string().max(1000).optional(),
}).refine(data => data.response_value !== undefined || data.response_text !== undefined, { message: 'Either response_value or response_text must be provided' });
/**
 ...
 */
export const UpdateAssessmentResponseOperationSchema = UpdateAssessmentResponseSchema.extend({
// Ensure at least one field is provided for update
}).refine(data => Object.keys(data).length > 0, {
    message: 'At least one field must be provided for update',
});

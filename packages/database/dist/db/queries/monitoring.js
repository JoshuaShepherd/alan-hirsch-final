...
import { auditLogs, communities, contentItems, organizations, userAnalyticsEvents, userAssessments, userProfiles, } from '../schema';
// ============================================================================
// QUERY LOGGING
// ============================================================================
/**
 ...
 */
export async function logQueryExecution(logEntry, context) {
    try {
 ...
 * Log database error
 */
export async function logDatabaseError(error, query, context) {
    try {
        await db.insert(auditLogs).values({
            userId: context.userId,
 ...
 */
export async function getQueryPerformanceMetrics(context, options = {}) {
    ...
    const [totalStats, slowestQueries, frequentQueries, errorStats] = await Promise.all([
        // Total query statistics
        db
            .select({
            totalQueries: count(auditLogs.id),
            averageTime: sql `COALESCE(AVG((${auditLogs.newValues}->>'duration')::numeric), 0)`,
        })
            .from(auditLogs)
        ...
        db
            .select({
            query: sql `${auditLogs.newValues}->>'query'`,
            averageTime: sql `AVG((${auditLogs.newValues}->>'duration')::numeric)`,
 ...
 */
export async function getDatabaseHealth(context) {
    const [connectionStats, slowQueryStats, errorStats, indexStats] = await Promise.all([
        // Connection statistics (simplified - would need actual connection monitoring)
        db
            .select({
            connectionCount: sql `1`, // Placeholder
            activeConnections: sql `1`, // Placeholder
        })
            .from(auditLogs)
            .limit(1),
        // Slow query count
        db
            ...
            .from(auditLogs)
            .where(and(sql `${auditLogs.action} = 'SLOW_QUERY'`, sql `${auditLogs.createdAt} >= NOW() - INTERVAL '1 hour'`)),
        // Error rate
        db
            .select({
            errorCount: count(auditLogs.id),
        ...
        db
            .select({
            table: sql `'user_profiles'`,
            index: sql `'idx_user_profiles_email'`,
            usage: sql `100`,
        })
            .from(auditLogs)
            .limit(1),
 ...
 */
export async function getAuditLogs(context, options = {}) {
    const { userId, action, resource, riskLevel, dateFrom, dateTo, limit = 50, offset = 0, } = options;
    const conditions = [];
    if (userId) {
 ...
 * Get security events
 */
export async function getSecurityEvents(context, options = {}) {
 ...
 */
export async function trackAnalyticsEvent(eventData, context) {
    try {
 ...
 */
export async function getUserAnalyticsEvents(userId, context, options = {}) {
    const { eventType, eventAction, dateFrom, dateTo, limit = 50, offset = 0, } = options;
    const conditions = [eq(userAnalyticsEvents.userId, userId)];
    if (eventType) {
        conditions.push(eq(userAnalyticsEvents.eventType, eventType));
    }
    if (eventAction) {
 ...
 * Get system metrics
 */
export async function getSystemMetrics(context) {
    const [userStats, contentStats, communityStats, organizationStats, assessmentStats, healthMetrics,] = await Promise.all([
        // User statistics
        db
            .select({
    ...
    const systemHealth = healthMetrics.errorRate > 0.1
        ? 'critical'

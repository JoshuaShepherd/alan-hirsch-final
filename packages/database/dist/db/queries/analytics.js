...
import { communities, communityMemberships, contentItems, organizationMemberships, organizations, userAnalyticsEvents, userAssessments, userContentInteractions, userProfiles, userSubscriptions, } from '../schema';
// ============================================================================
 ...
 */
export async function getUserEngagementMetrics(userId, context, options = {}) {
    ...
    const [totalStats, eventTypeStats, dailyStats] = await Promise.all([
        ...
        db
            .select({
            day: sql `DATE(${userAnalyticsEvents.createdAt})`,
            count: count(userAnalyticsEvents.id),
        })
            .from(userAnalyticsEvents)
            .where(and(...conditions))
 ...
 */
export async function getContentPerformanceAnalytics(authorId, context, options = {}) {
    ...
    return {
        ...
        contentEngagementRate: totalViews > 0 ? (totalLikes + totalShares) / totalViews : 0,
    };
}
/**
 * Get community engagement analytics
 */
export async function getCommunityEngagementAnalytics(communityId, context, options = {}) {
    ...
    const [memberStats, postStats, topContributors] = await Promise.all([
        ...
        db
            ...
            totalComments: count(sql `CASE WHEN ${userContentInteractions.interactionType} = 'comment' THEN 1 END`),
            totalVotes: count(sql `CASE WHEN ${userContentInteractions.interactionType} = 'vote' THEN 1 END`),
        })
            .from(userContentInteractions)
            .where(and(eq(userContentInteractions.communityId, communityId), timeCondition)),
        // Top contributors
        db
            .select({
            ...
            .from(userContentInteractions)
            .leftJoin(userProfiles, eq(userContentInteractions.userId, userProfiles.id))
            .where(and(eq(userContentInteractions.communityId, communityId), timeCondition))
 ...
 */
export async function getOrganizationAnalytics(organizationId, context, options = {}) {
    const { timeframe = 'month' } = options;
    const timeCondition = {
    ...
    const [memberStats, subscriptionStats, topMembers] = await Promise.all([
        ...
        db
            ...
            activeMembers: count(sql `CASE WHEN ${organizationMemberships.status} = 'active' THEN 1 END`),
            newMembers: count(sql `CASE WHEN ${timeCondition} THEN 1 END`),
        })
        ...
        db
            ...
            activeSubscriptions: count(sql `CASE WHEN ${userSubscriptions.status} = 'active' THEN 1 END`),
            monthlyRevenue: sql `COALESCE(SUM(CASE WHEN ${userSubscriptions.status} = 'active' THEN ${userSubscriptions.amount} ELSE 0 END), 0)`,
            totalRevenue: sql `COALESCE(SUM(${userSubscriptions.totalRevenue}), 0)`,
        })
        ...
        db
            ...
            .leftJoin(userAnalyticsEvents, eq(userProfiles.id, userAnalyticsEvents.userId))
 ...
 */
export async function getPlatformAnalytics(context, options = {}) {
    const { timeframe = 'month' } = options;
    const timeCondition = {
    ...
    const [userStats, contentStats, communityStats, organizationStats, subscriptionStats, topCategories, topCommunities,] = await Promise.all([
        // User statistics
        db
            .select({
            totalUsers: count(userProfiles.id),
            activeUsers: count(sql `CASE WHEN ${userProfiles.lastActiveAt} >= NOW() - INTERVAL '30 days' THEN 1 END`),
        ...
        db
            ...
            activeCommunities: count(sql `CASE WHEN ${communities.isActive} = true THEN 1 END`),
        })
            .from(communities),
        ...
        db
            ...
            activeOrganizations: count(sql `CASE WHEN ${organizations.status} = 'active' THEN 1 END`),
        })
            .from(organizations),
        // Subscription statistics
        db
            ...
            totalSubscriptions: count(userSubscriptions.id),
            activeSubscriptions: count(sql `CASE WHEN ${userSubscriptions.status} = 'active' THEN 1 END`),
            monthlyRecurringRevenue: sql `COALESCE(SUM(CASE WHEN ${userSubscriptions.status} = 'active' THEN ${userSubscriptions.amount} ELSE 0 END), 0)`,
        })
            .from(userSubscriptions),
        // Top content categories
        db
            .select({
            categoryId: sql `primary_category_id`,
            categoryName: sql `'Unknown'`, // Would need to join with content_categories
            contentCount: count(contentItems.id),
        })
            .from(contentItems)
 ...
 */
export async function getAssessmentAnalytics(context, options = {}) {
    ...
    const [totalStats, giftStats, culturalStats] = await Promise.all([
        ...
        db
            ...
            completedAssessments: count(sql `CASE WHEN ${userAssessments.completedAt} IS NOT NULL THEN 1 END`),
            averageScore: sql `COALESCE(AVG(${userAssessments.totalScore}), 0)`,
            averageCompletionTime: sql `COALESCE(AVG(${userAssessments.completionTime}), 0)`,
        })
        ...
        db
            ...
            .from(userAssessments)
            .where(and(...conditions, sql `${userAssessments.completedAt} IS NOT NULL`, sql `${userAssessments.primaryGift} IS NOT NULL`))
        ...
        db
            ...
            .where(and(...conditions, sql `${userAssessments.completedAt} IS NOT NULL`))
    ...
    const totalCulturalCount = culturalStats.reduce((sum, item) => sum + item.count, 0);
    return {
        totalAssessments,
        ...
        culturalDistribution: culturalStats.map(item => ({
            ...
            count: item.count,
            percentage: totalCulturalCount > 0 ? (item.count / totalCulturalCount) * 100 : 0,
        })),
    };
}

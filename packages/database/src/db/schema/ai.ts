...
export const aiConversations = pgTable('ai_conversations', {
  ...
  // Content References
  referencedContent: jsonb('referenced_content').$type<string[]>().default([]), // Content IDs
...
export const aiMessages = pgTable('ai_messages', {
  ...
  // Content References
  citedContent: jsonb('cited_content').$type<{
    contentId: string;
    title: string;
...
export const theologicalConcepts = pgTable('theological_concepts', {
  ...
  // Theological Context
  theologicalTradition: jsonb('theological_tradition').$type<string[]>().default([]),
  biblicalReferences: jsonb('biblical_references').$type<string[]>().default([]),
  historicalPeriod: text('historical_period'),
  
  // Relationships
  relatedConcepts: jsonb('related_concepts').$type<string[]>().default([]), // Concept IDs
  synonyms: jsonb('synonyms').$type<string[]>().default([]),
  
  // APEST Relevance
  apestRelevance: jsonb('apest_relevance').$type<{
    apostolic: number;
    prophetic: number;
    evangelistic: number;
    shepherding: number;
    teaching: number;
  }>().default({ apostolic: 5, prophetic: 5, evangelistic: 5, shepherding: 5, teaching: 5 }),

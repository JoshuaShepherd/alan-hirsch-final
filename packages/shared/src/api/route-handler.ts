 ...
 */
export function createRouteHandler<TInput, TOutput>(config: {
 ...
 */
export function createPaginatedRouteHandler<TInput, TOutput>(config: {
  inputSchema?: ZodSchema<TInput>;
  outputSchema?: ZodSchema<TOutput>;
  requireAuth?: boolean;
  method?: 'GET' | 'POST' | 'PUT' | 'DELETE' | 'PATCH';
    ...
    try {
      ...
      if (config.outputSchema) {
        validatedItems = result.items.map(item =>
          config.outputSchema!.parse(item)
        );
      }
 ...
 */
export function extractRouteParams<T extends Record<string, ZodSchema>>(
 ...
 */
export function createQuerySchema<T extends z.ZodRawShape>(baseSchema: T) {
  return z.object({
    ...baseSchema,
 ...
 */
export function createBulkSchema<T>(itemSchema: ZodSchema<T>) {
  return z.object({

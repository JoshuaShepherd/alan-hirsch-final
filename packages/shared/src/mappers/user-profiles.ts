...
import type { userProfiles } from '@platform/database';

// Drizzle row types
type UserProfileRow = typeof userProfiles.$inferSelect;
type NewUserProfileRow = typeof userProfiles.$inferInsert;
...
export function toUserProfileEntity(row: UserProfileRow): UserProfileEntity {
  ...
  if (
    ...
    if (!validation.success) {
      throw new Error(
        `UserProfileEntity validation failed: ${validation.error.message}`
      );
    }
  }
...
export function toUserProfileResponseDTO(
  ...
  if (
    ...
    if (!validation.success) {
      throw new Error(
        `UserProfileResponse validation failed: ${validation.error.message}`
      );
    }
  }
...
export function fromCreateUserProfile(
  ...
  if (
    ...
    if (!validation.success) {
      throw new Error(
        `CreateUserProfile validation failed: ${validation.error.message}`
      );
    }
  }
...
export function fromUpdateUserProfile(
  ...
  if (
    ...
    if (!validation.success) {
      throw new Error(
        `UpdateUserProfile validation failed: ${validation.error.message}`
      );
    }
  }

  const updateData: Partial<NewUserProfileRow> = {
    updatedAt: new Date(),
  };
...
export type UserProfileMapperInput = UserProfileRow;
export type UserProfileMapperOutput = UserProfileEntity;
export type UserProfileResponseMapperInput = UserProfileRow & {
  organization?: { id: string; name: string; slug: string; type: string };
  subscription?: {
    id: string;
    planName: string;
...
export type UserProfileResponseMapperOutput = UserProfileResponse;

 ...
 */
export function validateIngressRequest(request, schema, context) {
    try {
        let requestData;
        ...
        if (error instanceof z.ZodError) {
            return {
                isValid: false,
                error: createMinistryPlatformError('Validation failed', 'Request data validation failed', 'VALIDATION_ERROR', context, {
        ...
        return {
            isValid: false,
            error: createMinistryPlatformError('Request parsing failed', 'Failed to parse request data', 'REQUEST_PARSING_ERROR', context, error instanceof Error ? error.message : 'Unknown error'),
        };
    }
}
// ============================================================================
 ...
 */
export function validateEgressResponse(data, schema, context) {
    try {
        ...
        if (error instanceof z.ZodError) {
            return {
                isValid: false,
                error: createMinistryPlatformError('Response validation failed', 'Response data validation failed', 'RESPONSE_VALIDATION_ERROR', context, {
                    errors: error.errors.map(err => ({
                        field: err.path.join('.'),
                        message: err.message,
                        code: err.code,
        ...
        return {
            isValid: false,
            error: createMinistryPlatformError('Response processing failed', 'Failed to process response data', 'RESPONSE_PROCESSING_ERROR', context, error instanceof Error ? error.message : 'Unknown error'),
        };
    }
}
// ============================================================================
    ...
    try {
        ...
        if (organizationContext) {
            ...
            if (validatedContext.organizationId !== context.organizationId) {
                return {
                    isValid: false,
                    error: createMinistryPlatformError('Organization access denied', 'You do not have access to the specified organization', 'ORGANIZATION_ACCESS_DENIED', context, {
        ...
        return {
            isValid: false,
            error: createMinistryPlatformError('Invalid organization context', 'Organization context validation failed', 'INVALID_ORGANIZATION_CONTEXT', context, error instanceof z.ZodError ? error.errors : undefined),
        };
    }
}
// ============================================================================
...
export function validateRoleBasedPermissions(requiredPermissions, context) {
    try {
        ...
        if (!hasAllPermissions) {
            return {
                isValid: false,
                error: createMinistryPlatformError('Insufficient permissions', `Required permissions: ${requiredPermissions.join(', ')}`, 'INSUFFICIENT_PERMISSIONS', context, {
                    organizationId: context.organizationId,
                    userRole: context.userRole,
                    requiredPermissions,
                    suggestedActions: [
        ...
        return {
            isValid: false,
            error: createMinistryPlatformError('Permission validation failed', 'Role-based permission validation failed', 'PERMISSION_VALIDATION_ERROR', context, error instanceof z.ZodError ? error.errors : undefined),
        };
    }
}
// ============================================================================
...
export function validateCrossEntityAccess(entityType, entityId, context) {
    try {
        ...
        if (!hasAccess) {
            return {
                isValid: false,
                error: createMinistryPlatformError('Entity access denied', `You do not have access to this ${entityType}`, 'ENTITY_ACCESS_DENIED', context, {
                    organizationId: context.organizationId,
                    userRole: context.userRole,
        ...
        return {
            isValid: false,
            error: createMinistryPlatformError('Cross-entity validation failed', 'Entity access validation failed', 'CROSS_ENTITY_VALIDATION_ERROR', context, error instanceof z.ZodError ? error.errors : undefined),
        };
    }
}
// ============================================================================
...
//# sourceMappingURL=ministry-platform-validation.js.map
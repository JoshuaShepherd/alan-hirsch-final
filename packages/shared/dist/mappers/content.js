 ...
 */
export function toContentItemResponseDTO(row, author, category) {
    return {
        id: row.id,
        title: row.title,
        slug: row.slug,
        ...
        visibility: (row.visibility === 'invite_only' ? 'private' : row.visibility) ??
            'public',
        ...
        attachments: Array.isArray(row.attachments)
            ? row.attachments.map((att) => att.url || att)
            : [],
 ...
 */
export function toContentItemWithDetailsDTO(contentRow, authorRow, categoryRow) {
    return toContentItemResponseDTO(contentRow, authorRow ? toUserProfileDTO(authorRow) : undefined, categoryRow ? toContentCategoryResponseDTO(categoryRow) : undefined);
}
/**
 * Map content series with author and category details
 */
export function toContentSeriesWithDetailsDTO(seriesRow, authorRow, categoryRow) {
    return {
        id: seriesRow.id,
        ...
        collaborators: seriesRow.collaborators ?? [],

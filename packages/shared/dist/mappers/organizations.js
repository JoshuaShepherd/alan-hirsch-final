import { createOrganizationMembershipSchema, createOrganizationSchema, organizationEntitySchema, organizationMembershipEntitySchema, organizationMembershipResponseSchema, organizationResponseSchema, updateOrganizationMembershipSchema, updateOrganizationSchema, } from '@platform/contracts';
/**
 ...
 */
export function toOrganizationEntity(row) {
    const entity = {
        id: row.id,
        name: row.name,
        slug: row.slug,
 ...
 */
export function toOrganizationResponseDTO(row) {
    ...
    const response = {
        ...
        statusDisplay: entity.status.charAt(0).toUpperCase() + entity.status.slice(1),
        licenseTypeDisplay: entity.licenseType.charAt(0).toUpperCase() + entity.licenseType.slice(1),
        // Related data
        owner: row.owner,
        members: row.members,
    };
 ...
 */
export function toOrganizationMembershipEntity(row) {
    const entity = {
        id: row.id,
        userId: row.userId,
    ...
    if (!validation.success) {
        console.error('Organization membership entity validation failed:', validation.error);
 ...
 */
export function toOrganizationMembershipResponseDTO(row) {
    ...
    if (!validation.success) {
        console.error('Organization membership response validation failed:', validation.error);
 ...
 */
export function fromCreateOrganization(data) {
    // Validate input
 ...
 */
export function fromUpdateOrganization(data) {
    // Validate input
 ...
 */
export function fromCreateOrganizationMembership(data) {
    // Validate input
    const validation = createOrganizationMembershipSchema.safeParse(data);
    if (!validation.success) {
        console.error('Create organization membership validation failed:', validation.error);
 ...
 */
export function fromUpdateOrganizationMembership(data) {
    // Validate input
    const validation = updateOrganizationMembershipSchema.safeParse(data);
    if (!validation.success) {
        console.error('Update organization membership validation failed:', validation.error);

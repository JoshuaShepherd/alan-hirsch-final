 ...
 */
export function calculateApestScores(questions, responses) {
    const scores = {
        apostolic: 0,
        prophetic: 0,
        evangelistic: 0,
    ...
    for (const dimension of APEST_DIMENSIONS) {
        const dimensionQuestions = questions.filter(q => q.apestDimension === dimension);
 ...
 */
export function determineApestGifts(scores) {
 ...
 */
export function calculateResponseConsistency(responses) {
 ...
 */
export function calculateCompletionPercentage(questions, responses) {
    const totalQuestions = questions.filter(q => q.isRequired).length;
    const answeredQuestions = responses.filter(r => !r.skipped && r.responseValue !== undefined).length;
 ...
 */
export function normalizeScores(rawScores, maxPossibleScores) {
    const normalized = {};
 ...
 */
export function calculateMaxPossibleScores(questions) {
    const maxScores = {};
 ...
 */
export function scoreApestAssessment(questions, responses) {
    // Calculate APEST scores
    const apestScores = calculateApestScores(questions, responses);
    // Calculate total score
    const totalScore = Object.values(apestScores).reduce((sum, score) => sum + score, 0);
    // Calculate max possible score
    const maxPossibleScores = calculateMaxPossibleScores(questions);
    const maxPossibleScore = Object.values(maxPossibleScores).reduce((sum, score) => sum + score, 0);
    // Normalize scores
    const normalizedScores = normalizeScores(apestScores, maxPossibleScores);
    // Determine gifts
    const gifts = determineApestGifts(apestScores);
    // Calculate completion percentage
    const completionPercentage = calculateCompletionPercentage(questions, responses);
 ...
 */
export function generateApestInsights(apestScores, primaryGift, secondaryGift) {
    const insights = [];
    // Primary gift insights
    const giftDescriptions = {
        apostolic: "Apostolic gifts focus on pioneering, vision-casting, and expanding God's kingdom into new territories. You are drawn to starting new things and reaching unreached people.",
        prophetic: "Prophetic gifts involve speaking truth, challenging the status quo, and calling people back to God's heart. You have a deep sense of what God is saying and aren't afraid to speak up.",
        evangelistic: "Evangelistic gifts center on sharing the gospel and connecting with people who don't yet know Christ. You naturally build bridges between the church and the world.",
        shepherding: "Shepherding gifts focus on caring for, nurturing, and protecting God's people. You have a heart for helping others grow and supporting them through difficult times.",
        teaching: "Teaching gifts involve helping people understand and apply God's Word. You have a passion for discipleship and helping others develop a solid biblical foundation.",
    };
    insights.push(giftDescriptions[primaryGift] || '');
    // Secondary gift insights
    if (secondaryGift && secondaryGift !== primaryGift) {
        insights.push(`Your secondary gift of ${secondaryGift} complements your primary ${primaryGift} gift, creating a well-rounded ministry profile.`);
    }
    // Balance insights
    const sortedScores = APEST_DIMENSIONS.map(dimension => ({
        dimension,
    ...
    if (highestScore - lowestScore > 20) {
        insights.push('Your profile shows strong specialization in certain areas. Consider how you can work with others who have complementary gifts.');
    }
    else {
        insights.push('You have a relatively balanced profile across all APEST dimensions, making you versatile in ministry contexts.');
    }
 ...
 */
export function generatePersonalizedRecommendations(apestScores, primaryGift, _secondaryGift) {
    ...
    APEST_DIMENSIONS.forEach(dimension => {
        ...
        if (score >= strengthThreshold) {
            recommendations.strengths.push(`Strong ${dimension} gifts - leverage this in your ministry`);
        }
    });
    ...
    APEST_DIMENSIONS.forEach(dimension => {
        ...
        if (score < growthThreshold) {
            recommendations.growthAreas.push(`Develop your ${dimension} gifts through intentional practice`);
        }
    });

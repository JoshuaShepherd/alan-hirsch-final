...
export function createApiResponseFetcher<T>(
  ...
  return async (url: string): Promise<T> => {
    const { apiClient } = await import('../api-client');
    const response = await apiClient.getWithValidation(url, apiResponseSchema);
    ...
    return {
      ...
      meta: {
        pagination: {
          ...
          has_next: response.meta.pagination.hasNext,
          has_prev: response.meta.pagination.hasPrev,
        },

'use client';
import { assessmentCardPropsSchema, validateComponentProps, } from '@/lib/types/component-props';
...
import { AlertCircle, Award, BookOpen, CheckCircle, Clock, Edit, Eye, Globe, MoreHorizontal, Target, Trash2, } from 'lucide-react';
export function AssessmentCard({ item: assessment, variant = 'default', showActions = true, showQuestionCount = true, showDuration = true, showValidityScores = false, showCulturalAdaptation = true, onSelect, onEdit, onDelete, onView, className, }) {
    // Runtime validation
    ...
    const getCulturalAdaptationColor = (adaptation) => {
        const adaptationColors = {
        ...
        return (adaptationColors[adaptation.toLowerCase()] || 'bg-gray-100 text-gray-800');
    };
    // Get scoring method display
    const getScoringMethodDisplay = (method) => {
        ...
        <div className="flex items-center space-x-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center space-x-1 mt-1">
              <Badge className={cn('text-xs', getAssessmentTypeColor(assessment.assessmentType))}>
                {assessment.assessmentType.replace('_', ' ')}
              </Badge>
          ...
          {showActions && (<DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                {onView && (<DropdownMenuItem onClick={() => onView(assessment)}>
                    <Eye className="mr-2 h-4 w-4"/>
                    View
                  </DropdownMenuItem>)}
                {onEdit && (<DropdownMenuItem onClick={() => onEdit(assessment)}>
    ...
    return (<Card className={cn('h-full', className)}>
        ...
        <div className="flex items-start justify-between">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              {variant === 'detailed' && assessment.description && (<p className="text-sm text-muted-foreground mt-1 line-clamp-2">
      ...
      <CardContent className="pb-3">
        {/* Description */}
        {variant === 'default' && assessment.description && (<p className="text-sm text-muted-foreground line-clamp-2 mb-3">
            {assessment.description}
          </p>)}

        {/* Stats */}
        {(showQuestionCount || showDuration) && (<div className="grid grid-cols-3 gap-2 mb-3">
            {showQuestionCount && (<div className="text-center">
                <div className="text-sm font-medium">
                  {assessment.questionsCount}
                </div>
                <div className="text-xs text-muted-foreground">Questions</div>
              </div>)}
            {showDuration && assessment.estimatedDuration && (<div className="text-center">
                <div className="text-sm font-medium">
        ...
        {/* Validity Scores */}
        {showValidityScores &&
            (assessment.validityScore || assessment.reliabilityScore) && (<div className="space-y-2 mb-3">
              {assessment.validityScore && (<div className="flex items-center justify-between">
                  <span className="text-sm font-medium">Validity Score</span>
                  <div className="flex items-center space-x-2">
                    <div className="w-16 bg-gray-200 rounded-full h-2">
                      <div className="bg-green-600 h-2 rounded-full" style={{ width: `${assessment.validityScore * 100}%` }}/>
                    </div>
              ...
              {assessment.reliabilityScore && (<div className="flex items-center justify-between">
        ...
        <div className="space-y-2">
          ...
          {showCulturalAdaptation && (<div className="flex items-center space-x-2">
              <Badge className={cn('text-xs', getCulturalAdaptationColor(assessment.culturalAdaptation))}>
                {assessment.culturalAdaptation.replace('_', ' ')}
              </Badge>
              {assessment.researchBacked && (<Badge variant="outline" className="text-xs">
        ...
        {variant === 'detailed' && (<div className="space-y-2 mt-3">
            {assessment.passingScore && (<div className="flex items-center text-xs text-muted-foreground">
                <Target className="mr-1 h-3 w-3"/>
                Passing Score: {assessment.passingScore}
              </div>)}
            {assessment.instructions && (<div className="flex items-center text-xs text-muted-foreground">
            ...
            {assessment.publishedAt && (<div className="flex items-center text-xs text-muted-foreground">
                <Clock className="mr-1 h-3 w-3"/>
                Published{' '}
      ...
      {variant === 'detailed' && (onView || onEdit) && (<CardFooter className="pt-3">
          <div className="flex space-x-2 w-full">
            {onView && (<Button variant="outline" size="sm" onClick={() => onView(assessment)} className="flex-1">
                <Eye className="mr-2 h-4 w-4"/>
                View Details
              </Button>)}
            {onEdit && (<Button size="sm" onClick={() => onEdit(assessment)} className="flex-1">
                <Edit className="mr-2 h-4 w-4"/>
                Edit
              </Button>)}
          </div>

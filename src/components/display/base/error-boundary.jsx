...
export class ErrorBoundary extends Component {
    ...
    render() {
        if (this.state.hasError) {
            ...
            // Default error UI
            return (<div className={cn('flex flex-col items-center justify-center min-h-[400px] p-8', this.props.className)}>
          ...
          {process.env['NODE_ENV'] === 'development' && this.state.error && (<details className="mt-6 max-w-2xl w-full">
              ...
              <Button onClick={this.handleReportBug} variant="outline" size="sm" className="mt-2">
...
export function ErrorDisplay({ error, title = 'Error', description, onRetry, onDismiss, className, }) {
    return (<Alert className={cn('border-destructive', className)}>
      <AlertTriangle className="h-4 w-4"/>
      <AlertTitle>{title}</AlertTitle>
      <AlertDescription className="mt-2">
        {description || error.message}
      </AlertDescription>
      {(onRetry || onDismiss) && (<div className="flex gap-2 mt-3">
          {onRetry && (<Button onClick={onRetry} size="sm" variant="outline">
              <RefreshCw className="mr-2 h-3 w-3"/>
              Retry
            </Button>)}
          {onDismiss && (<Button onClick={onDismiss} size="sm" variant="ghost">
              Dismiss
            </Button>)}
        </div>)}
    </Alert>);
...
export function PermissionError({ onRetry, className }) {
    return (<div className={cn('flex flex-col items-center justify-center py-12 text-center', className)}>
      <div className="text-6xl mb-4">🔒</div>
...
export function NotFoundError({ resource = 'Resource', onRetry, className, }) {
    return (<div className={cn('flex flex-col items-center justify-center py-12 text-center', className)}>
      <div className="text-6xl mb-4">🔍</div>

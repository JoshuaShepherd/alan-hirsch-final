...
import { BaseEntity, EntityCardProps } from '@/lib/types/component-props';
...
export function EntityCard<T extends BaseEntity>({
  ...
  const description =
    ...
    (entity as any)['excerpt'] ||
    (entity as any)['bio'] ||
    '';
  const status =
    (entity as any)['status'] ||
    (entity as any)['accountStatus'] ||
    (entity as any)['visibility'] ||
    'active';
  const createdAt =
    (entity as any)['createdAt'] ||
    (entity as any)['joinedAt'] ||
    (entity as any)['publishedAt'];
  const updatedAt =
    (entity as any)['updatedAt'] || (entity as any)['lastActiveAt'];
  ...
  const stats = (() => {
    ...
    switch (entityType) {
      case 'user':
        return [
          { label: 'Ministry Years', value: e['yearsInMinistry'] || 0 },
          { label: 'Leader Tier', value: e['leaderTier'] || 'N/A' },
          {
            label: 'Subscription',
            value: e['subscriptionTier'] || 'free',
          },
        ];
      case 'organization':
        return [
          { label: 'Members', value: e['maxUsers'] || 1 },
          { label: 'Type', value: e['organizationType'] || 'N/A' },
          { label: 'Status', value: e['status'] || 'active' },
        ];
      case 'assessment':
        return [
          { label: 'Questions', value: e['questionsCount'] || 0 },
          {
            label: 'Duration',
            value: `${e['estimatedDuration'] || 0}min`,
          },
          { label: 'Type', value: e['assessmentType'] || 'N/A' },
        ];
      case 'content':
        return [
          { label: 'Views', value: e['viewCount'] || 0, type: 'compact' },
          { label: 'Likes', value: e['likeCount'] || 0, type: 'compact' },
          { label: 'Type', value: e['contentType'] || 'N/A' },
        ];
      case 'community':
        return [
          {
            label: 'Members',
            value: e['currentMemberCount'] || 0,
            type: 'compact',
          },
          {
            label: 'Posts',
            value: e['totalPostsCount'] || 0,
            type: 'compact',
          },
          { label: 'Type', value: e['communityType'] || 'N/A' },
        ];
      case 'subscription':
        return [
          {
          ...
          { label: 'Type', value: e['planType'] || 'N/A' },
          { label: 'Trial', value: e['trialDays'] || 0, suffix: ' days' },
        ];
      case 'collaboration':
        return [
          {
            label: 'Collaborators',
            value: e['collaborators']?.length || 0,
          },
          { label: 'Status', value: e['status'] || 'planning' },
          { label: 'Type', value: e['collaborationType'] || 'N/A' },
        ];
      case 'post':
        return [
          { label: 'Upvotes', value: e['upvotes'] || 0, type: 'compact' },
          {
            label: 'Replies',
            value: e['replyCount'] || 0,
            type: 'compact',
          },
          { label: 'Type', value: e['postType'] || 'N/A' },
        ];
      default:
        return [];
    }

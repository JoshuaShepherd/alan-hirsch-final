...
export function UserAvatar({ user, size = 'md', showStatus = false, onClick, className, }) {
    ...
    const getOnlineStatusColor = (isActive) => {
    ...
    return (<div className={cn('relative inline-block', className)}>
      ...
      {/* Status Indicator */}
      {showStatus && (<div className={cn('absolute -bottom-0.5 -right-0.5 rounded-full border-2 border-white', config.badge, getOnlineStatusColor(user.accountStatus === 'active'))}/>)}
    </div>);
}
export function UserGroupAvatar({ users, maxVisible = 3, size = 'md', className, }) {
    const sizeConfig = {
        sm: 'h-6 w-6',
        md: 'h-8 w-8',
        lg: 'h-12 w-12',
    ...
    return (<div className={cn('flex -space-x-2', className)}>
      {visibleUsers.map((user, index) => {
            const displayName = user.displayName || `${user.firstName} ${user.lastName}`;
            const initials = `${user.firstName?.[0] || ''}${user.lastName?.[0] || ''}`.toUpperCase();
            return (<Avatar key={user.id} className={cn(sizeConfig[size], 'border-2 border-white')}>
            <AvatarImage src={user.avatarUrl} alt={displayName}/>
            <AvatarFallback className="text-xs">{initials}</AvatarFallback>
          </Avatar>);
        })}

      {remainingCount > 0 && (<Avatar className={cn(sizeConfig[size], 'border-2 border-white bg-muted')}>

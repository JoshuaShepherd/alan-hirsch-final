...
export interface SearchFormProps<T extends BaseSearchInput> {
 ...
 */
export function SearchForm<T extends BaseSearchInput>({
  schema,
  onSearch,
  onReset,
  defaultValues,
  ...
  const debouncedSearch = useDebouncedCallback((values: T) => {
    setIsSearching(true);
    onSearch(values);
  ...
  useEffect(() => {
    ...
    params.forEach((value, key) => {
      ...
      if (Array.isArray(currentValue)) {
        urlData[fieldName] = value.split(',') as T[keyof T];
      } else if (typeof currentValue === 'number') {
        urlData[fieldName] = Number(value) as T[keyof T];
      } else if (typeof currentValue === 'boolean') {
        urlData[fieldName] = (value === 'true') as T[keyof T];
      } else {
        urlData[fieldName] = value as T[keyof T];
      }
    });
  ...
  const removeFilter = (filterKey: string) => {
    form.setValue(filterKey as keyof T, undefined as T[keyof T]);
  };
          ...
          {showSorting && (
            <FormFieldGroup columns={2}>
              ...
              >
                <Select
                  onValueChange={value =>
                    form.setValue('sortOrder', value as 'asc' | 'desc')
                  }
                >
                  <SelectTrigger>
...
export function ContentSearchForm(
  props: Omit<SearchFormProps<ContentSearchInput>, 'schema'>
) {
  return <SearchForm {...props} schema={contentSearchSchema} />;
}

export function UserSearchForm(
  props: Omit<SearchFormProps<UserSearchInput>, 'schema'>
) {
  return <SearchForm {...props} schema={userSearchSchema} />;
}

export function AssessmentSearchForm(
  props: Omit<SearchFormProps<AssessmentSearchInput>, 'schema'>
) {

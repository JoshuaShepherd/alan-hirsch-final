// Auto-generated collection route for FeatureFlags
...
// import { featureflagsQuerySchema, featureflagsInputSchema } from '@/lib/contracts';
// import { featureflagsArrayToApi, featureflagsFromApi } from '@/lib/mappers';
// import type { FeatureFlagsSelect, FeatureFlagsInsert, FeatureFlagsResponse } from '@/lib/types';
...
// GET /api/featureFlags - List entities with filtering, pagination, sorting
export async function GET(request: NextRequest) {
  try {
    ...
    console.error('Error fetching FeatureFlags list:', error);
    ...
    return NextResponse.json({
      ...
      error: {
        code: 'INTERNAL_ERROR',
        message: 'Failed to fetch FeatureFlags list',
      },
    }, { status: 500 });
  }
}

// POST /api/featureFlags - Create new entity
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      success: true,
      data: result,
      message: 'FeatureFlags created successfully',
    }, { status: 201 });
  } catch (error) {
    console.error('Error creating FeatureFlags:', error);
    ...
    return NextResponse.json({
      ...
      error: {
        code: 'INTERNAL_ERROR',
        message: 'Failed to create FeatureFlags',
      },
    }, { status: 500 });
  }
}
// Auto-generated collection route for UserConsents
...
// import { userconsentsQuerySchema, userconsentsInputSchema } from '@/lib/contracts';
// import { userconsentsArrayToApi, userconsentsFromApi } from '@/lib/mappers';
// import type { UserConsentsSelect, UserConsentsInsert, UserConsentsResponse } from '@/lib/types';
...
// GET /api/userConsents - List entities with filtering, pagination, sorting
export async function GET(request: NextRequest) {
  try {
    ...
    console.error('Error fetching UserConsents list:', error);
    ...
    return NextResponse.json({
      ...
      error: {
        code: 'INTERNAL_ERROR',
        message: 'Failed to fetch UserConsents list',
      },
    }, { status: 500 });
  }
}

// POST /api/userConsents - Create new entity
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      success: true,
      data: result,
      message: 'UserConsents created successfully',
    }, { status: 201 });
  } catch (error) {
    console.error('Error creating UserConsents:', error);
    ...
    return NextResponse.json({
      ...
      error: {
        code: 'INTERNAL_ERROR',
        message: 'Failed to create UserConsents',
      },
    }, { status: 500 });
  }
}
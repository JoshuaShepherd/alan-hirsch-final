// Auto-generated collection route for AiContentJobs
...
// import { aicontentjobsQuerySchema, aicontentjobsInputSchema } from '@/lib/contracts';
// import { aicontentjobsArrayToApi, aicontentjobsFromApi } from '@/lib/mappers';
// import type { AiContentJobsSelect, AiContentJobsInsert, AiContentJobsResponse } from '@/lib/types';
...
// GET /api/aiContentJobs - List entities with filtering, pagination, sorting
export async function GET(request: NextRequest) {
  try {
    ...
    console.error('Error fetching AiContentJobs list:', error);
    ...
    return NextResponse.json({
      ...
      error: {
        code: 'INTERNAL_ERROR',
        message: 'Failed to fetch AiContentJobs list',
      },
    }, { status: 500 });
  }
}

// POST /api/aiContentJobs - Create new entity
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      success: true,
      data: result,
      message: 'AiContentJobs created successfully',
    }, { status: 201 });
  } catch (error) {
    console.error('Error creating AiContentJobs:', error);
    ...
    return NextResponse.json({
      ...
      error: {
        code: 'INTERNAL_ERROR',
        message: 'Failed to create AiContentJobs',
      },
    }, { status: 500 });
  }
}
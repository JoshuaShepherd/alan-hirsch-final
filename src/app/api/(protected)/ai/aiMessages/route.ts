// Auto-generated collection route for AiMessages
...
// import { aimessagesQuerySchema, aimessagesInputSchema } from '@/lib/contracts';
// import { aimessagesArrayToApi, aimessagesFromApi } from '@/lib/mappers';
// import type { AiMessagesSelect, AiMessagesInsert, AiMessagesResponse } from '@/lib/types';
...
// GET /api/aiMessages - List entities with filtering, pagination, sorting
export async function GET(request: NextRequest) {
  try {
    ...
    console.error('Error fetching AiMessages list:', error);
    ...
    return NextResponse.json({
      ...
      error: {
        code: 'INTERNAL_ERROR',
        message: 'Failed to fetch AiMessages list',
      },
    }, { status: 500 });
  }
}

// POST /api/aiMessages - Create new entity
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      success: true,
      data: result,
      message: 'AiMessages created successfully',
    }, { status: 201 });
  } catch (error) {
    console.error('Error creating AiMessages:', error);
    ...
    return NextResponse.json({
      ...
      error: {
        code: 'INTERNAL_ERROR',
        message: 'Failed to create AiMessages',
      },
    }, { status: 500 });
  }
}
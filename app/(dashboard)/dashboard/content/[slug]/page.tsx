...
import { Heart, Share2, Bookmark, MessageCircle, Clock, Eye } from 'lucide-react';
...
export default async function ContentViewPage({ 
  ...
  // Fetch content item
  const response = await fetch(`${process.env['NEXT_PUBLIC_APP_URL'] || 'http://localhost:3000'}/api/content?slug=${slug}`, {
    headers: {
  ...
  try {
    await fetch(`${process.env['NEXT_PUBLIC_APP_URL'] || 'http://localhost:3000'}/api/content/${content.id}/view`, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${(await supabase.auth.getSession()).data.session?.access_token}`
      }
              ...
              <div className="flex items-center space-x-2">
                {content.author?.avatarUrl ? (
                  <img 
                    ...
                    alt={content.author.displayName ?? `${content.author.firstName ?? '(unknown)'} ${content.author.lastName ?? '(unknown)'}`}
                    className="w-8 h-8 rounded-full"
                  />
                ) : (
                  <div className="w-8 h-8 bg-gray-300 rounded-full flex items-center justify-center">
                    <span className="text-sm font-medium">
                      {(content.author?.firstName ?? 'U')[0]}{(content.author?.lastName ?? 'U')[0]}
                    </span>
                  </div>
                )}
                <div>
                  <div className="font-medium">
                    {content.author?.displayName ?? `${content.author?.firstName ?? '(unknown)'} ${content.author?.lastName ?? '(unknown)'}`}
                  </div>

...
export function withValidation<T>(
  schema: ZodSchema<T>,
  handler: (data: T, context: MiddlewareContext, request: NextRequest) => Promise<Response>
) {
...
export function withInputOutputValidation<TInput, TOutput>(
  ...
  handler: (data: TInput, context: MiddlewareContext, request: NextRequest) => Promise<TOutput>
) {
  return async (request: NextRequest) => {
    try {
      // Get authenticated user
...
export function withPaginationValidation<TInput, TOutput>(
  inputSchema: ZodSchema<TInput>,
  outputSchema: ZodSchema<TOutput>,
  handler: (data: TInput, context: MiddlewareContext, request: NextRequest) => Promise<{ items: TOutput[], total: number }>
) {
  return async (request: NextRequest) => {
    try {
      // Get authenticated user

...
export const assessments = pgTable('assessments', {
  ...
  assessmentType: text('assessment_type', {
    enum: ['apest', 'mdna', 'cultural_intelligence', 'leadership_style', 'spiritual_gifts', 'other']
  }).notNull(),
  ...
  culturalAdaptation: text('cultural_adaptation', {
    enum: ['western', 'eastern', 'african', 'latin_american', 'middle_eastern', 'oceanic', 'universal']
  }).default('universal'),
...
export const userAssessments = pgTable('user_assessments', {
  ...
  // Quality Metrics
  responseConsistency: decimal('response_consistency', { precision: 3, scale: 2 }),
  completionTime: integer('completion_time'), // minutes
  confidenceLevel: integer('confidence_level'), // 1-5 scale
  
  // Cultural Adjustment
  culturalAdjustmentApplied: boolean('cultural_adjustment_applied').default(false),
  culturalAdjustmentFactor: decimal('cultural_adjustment_factor', { precision: 3, scale: 2 }),
...
export const assessmentResponses = pgTable('assessment_responses', {
  id: uuid('id').primaryKey().defaultRandom(),
  userAssessmentId: uuid('user_assessment_id').notNull().references(() => userAssessments.id, { onDelete: 'cascade' }),
